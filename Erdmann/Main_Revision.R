...
trueMean <- function(scenario, tp, output_name){
  ...
  save(true_mean, 
       ...
       # ohne absolute pfade
       file = paste0("aus_sim_repo/saves/", output_name, ".rda"))
  return(true_mean)
}
...
CalculateMeans <- function(repN, true_mean, scenario, output_name, boxplot = FALSE, term_event, stop = FALSE) {
  tp <- c(40, 60, 80, 100)
  if (term_event) {
...
load(file = paste0("aus_sim_repo/results/trueMeanMaster.rda"))
